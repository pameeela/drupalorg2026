...
// few slides, so we duplicate the slides up to this minimum.
const MIN_SLIDES = 5;

document.querySelectorAll(".carousel-tabs").forEach((carousel) => {
  const swiperEl = carousel.querySelector(".swiper");
  const wrapper = swiperEl && swiperEl.querySelector(".swiper-wrapper");
  const nav = carousel.querySelector(".carousel-tabs__nav");
  ...
  // Hoist each slide's nav (icon + title + description) into the
  ...
  const navButtons = originalSlides.map((slide) => {
    ...
    button.type = "button";
    button.className = "carousel-tabs__feature";
  ...
  const detail = carousel.querySelector(".carousel-tabs__detail");

  // Duplicate the slide set when there are too few for a both-sides peek.
  ...
  const setActive = () => {
    ...
    if (detail) {
      detail.innerHTML =
        '<div class="carousel-tabs__detail-feature">' +
        featureHTML[activeFeature] +
        "</div>" +
        '<div class="carousel-tabs__detail-cta">' +

...
```twig
{% include '@drupalorg/components/icon/icon.twig' with {
  weight: 'bold',
  icon: icon
} %}
```
...
**Note**: These commands should be run from the Drupalorg theme directory (`web/themes/custom/drupalorg/`). Running these commands ensures that:

# Drupalorg Theme - Agent Rules

This document contains coding rules and conventions for the Drupalorg theme that AI agents should follow when making changes.
...
**Note**: Even though Drupal core templates sometimes use `<div{{ attributes }}>`, this pattern is not allowed in Drupalorg theme components.
...
```twig
{% include '@drupalorg_theme/components/icon/icon.twig' with {
  weight: 'bold',
  icon: icon
} %}
```
...
**Rule**: After completing any changes to the Drupalorg theme, always run `npm run format` and `npm run build` to ensure code is properly formatted and the build artifacts are up to date.
...
**Note**: These commands should be run from the Drupalorg theme directory (`web/themes/drupalorg_theme/`). Running these commands ensures that:
